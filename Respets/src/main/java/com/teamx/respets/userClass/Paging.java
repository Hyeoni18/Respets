...
////////////////////////////////////////////////////현휘 (주소, 태그 )
	...
	private String busAllPaging(int currentGroup, int totalPage, String boardName, String search) {
		...
		sb.append("<div class='dataTables_paginate paging_simple_numbers'" + "id='selection-datatable_paginate'>"
				+ "<ul class='pagination pagination-rounded'>");
		if (start != 1) {
			sb.append("<li class='paginate_button page-item previous disabled' id='selection-datatable_previous'>"
					+ "<a href='javascript:void(0)' aria-controls='selection-datatable' data-dt-idx='0' tabindex='0' class='page-link' onclick=\"butListPaging('" + 
					(start - 1) + "','','"+search+"')\">");
			sb.append("<i class='mdi mdi-chevron-left'></i></a></li>");
		} // method End
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { // 현재 페이지가 아닌 경우 링크 처리
				sb.append("<li class='paginate_button page-item'><a href='javascript:void(0)' ' aria-controls='basic-datatable' data-dt-idx='"+i+"' tabindex='0' "
						+ "class='page-link' onclick=\"butListPaging('" + i + "','','"+search+"')\">");
				sb.append(i + "</a></li>");
			} else { // 현재 페이지인 경우 링크 해제
				sb.append("<li class='paginate_button page-item active'><a href='#' aria-controls='basic-datatable' data-dt-idx='"+i+"' tabindex='0' "
						+ "class='page-link'> " + i + "  </a></li>");
			} // else End
		} // for End

		if (end != totalPage) {
			sb.append("<li class='paginate_button page-item previous' id='selection-datatable_previous'><a href='javascript:void(0)' "
					+ "aria-controls='selection-datatable' data-dt-idx='6' tabindex='0' class='page-link' onclick=\"butListPaging('" + 
					(end + 1) + "','','"+search+"')\">");
			sb.append("<i class='mdi mdi-chevron-right'></i></a></li>");
		} // if End
		sb.append("</ul>");
